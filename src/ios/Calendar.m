...
-(EKCalendar*)findEKCalendar: (NSString *)calendarName {
    ...
    if ([self.eventStore respondsToSelector:@selector(calendarWithEventStore:)]) {
...
-(EKSource*)findEKSource {
    EKSource *source = self.eventStore.defaultCalendarForNewEvents.source;
...
- (void)createEventInNamedCalendar:(CDVInvokedUrlCommand*)command {
    ...
    if (calendar == nil) {
        [self createCalendarNamed:calendarName];
...
- (EKCalendar *)createCalendarNamed:(NSString *)calendarName {
    EKCalendar *calendar = [self findEKCalendar:calendarName];
    ...
    if (calendar == nil) {
        ...
        NSLog(@"created calendar: %@ (error: %@)", calendar.title, error);
    }
